        ...
        <footer className="bg-[#2C2C2C] text-white pt-20 pb-10 px-4 sm:px-6">
            <div className="mx-auto max-w-7xl">
                <div className="grid gap-12 md:grid-cols-4">
                    ...
                    <div className="col-span-1 md:col-span-2">
                        <Link to="/" className="inline-block group">
                            <h3 className="text-3xl font-light tracking-tight mb-4">
                                Beauty<span className="text-[#C5A28A] group-hover:text-[#B88E72] transition-colors">Book</span>
                            </h3>
                        </Link>
                        ...
                        <div className="mt-8 flex gap-4">
                            {[
                                ...
                                >
                                    ...
                                    <span className="text-xs font-light tracking-wider">{social.icon}</span>
                                </motion.a>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Company Links */}
                    <div>
                        <h4 className="text-sm tracking-widest uppercase mb-6 text-[#E8D5C4] font-light">Ընկերություն</h4>
                        <ul className="space-y-4">
                            {['Մեր մասին', 'Կարիերա', 'Բլոգ', 'Մամուլ'].map((item) => (
                                <li key={item}>
                                    <Link
                                        to={`/${item.toLowerCase().replace(' ', '-')}`}
                                        className="text-sm text-[#E8D5C4] hover:text-white
                                                 transition-colors duration-300 font-light inline-block
                                                 hover:translate-x-1 transform transition-all"
                                    >
                                        {item}
                                    </Link>
                                </li>
                    ...
                    {/* Legal Links */}
                    <div>
                        <h4 className="text-sm tracking-widest uppercase mb-6 text-[#E8D5C4] font-light">Իրավական</h4>
                        <ul className="space-y-4">
                            {['Գաղտնիություն', 'Պայմաններ', 'Cookies', 'Լիցենզիաներ'].map((item) => (
                                <li key={item}>
                                    <Link
                ...
                <div className="mt-12 pt-8 border-t border-[#E8D5C4]/10">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="text-sm text-[#E8D5C4] font-light order-2 md:order-1">
                            © {new Date().getFullYear()} BeautyBook. Բոլոր իրավունքները պաշտպանված են։
                        </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-6 md:gap-8 text-sm text-[#E8D5C4] font-light order-1 md:order-2">
                            ...
                            <motion.a
                                href="mailto:info@beautybook.am"
                                whileHover={{ scale: 1.05 }}
                                className="flex items-center gap-2 hover:text-white transition-colors"
                            >
                                <span className="w-1 h-1 rounded-full bg-[#C5A28A]" />
                                <span>info@beautybook.am</span>
                            </motion.a>
